...
async function runV2Pipeline(video: ChannelVideo): Promise<VideoAnalysis> {
  ...
  if (ingest.error || !ingest.data?.job_id) {
    throw new Error(`v2_ingest_youtube_video failed: ${getErrorMessage(ingest.error || 'missing job_id')}`);
  }
  ...
  for (const fn of chain) {
    ...
    if (response.error || (response.data && response.data.success === false)) {
      throw new Error(`${fn} failed: ${getErrorMessage(response.error || response.data?.error)}`);
    }
  }
...
export async function publishCuratedVideos(
  ...
  const affectedPlaylists = Array.from(new Set(Object.values(input.mappings)));
  return {
    success: true,
    message: 'Classifications are reviewed through the v2 classification flow.',
    publishedCount: input.videoIds.length,
    affectedPlaylists,
